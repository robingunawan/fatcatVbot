...
async def temp_mail(client, message):
    ...
    url = f"https://api.botcahx.eu.org/api/tools/create-temp-mail?apikey=moire_mor"
    data = fetch_json(url)
...
async def check_mail(client, message):
    ...
    url = f"https://api.botcahx.eu.org/api/tools/cek-msg-tmp-mail?email={email}&apikey=moire_mor"
    data = fetch_json(url)
